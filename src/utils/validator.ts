    ...
    validationSchema.forEach(o => {
        if (objectToValidate[o.param]) {
            if (typeof objectToValidate[o.param] !== o.type) {
                errors.push({ param: o.param, error: o.error, errorType: 'INCORRECT_TYPE' });
            }
            if (o.regex && !o.regex.test(objectToValidate[o.param])) {
                errors.push({ param: o.param, error: o.error, errorType: 'REGEX_UNMATCH' });
            }
            if (o.customValidate && (typeof o.customValidate === "function" && !o.customValidate(objectToValidate[o.param]))) {
                errors.push({ param: o.param, error: o.error, errorType: 'CUSTOM_VALIDATE_FAILED' });
            }
        } else {
            if (o.required) {
                errors.push({ param: o.param, error: o.error, errorType: 'IS_REQUIRED' });
            }
        }
    });
    return errors;
